...
// --- Core Visual Elements ---
    ...
    VisualNode::VisualNode(int value, sf::Font& font, sf::Vector2f startPos) 
    ...
    void VisualNode:: setValue(int val) {
        value = val;
        m_graphic.setText(std::to_string(val));
    }

    void VisualNode:: updateTextPos() {
        if (index >= 0) {
    ...
    void VisualNode:: HandleEvent(const sf::Event& event, const sf::RenderWindow& window) {
    ...
    void VisualNode:: update(const sf::RenderWindow& window, float dt) {
        if (isDragging) {
            sf::Vector2f mousePos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
    ...
    Connector:: Connector(VisualNode& p1, VisualNode& p2) {
        start_UID = p1.UID;
        end_UID = p2.UID;
    }

    void Connector::update(VisualNode& startNode, VisualNode& endNode, float dt) {
 ...
 void ResolveCollisions(std::vector<VisualNode>& nodes, const sf::RenderWindow& window, float leftBound, float rightBound ) {
...
// --- Basic UI Components ---
    ...
    TextBox::TextBox(sf::Font& font, sf::Vector2f pos, float length, float width) {
    ...
    void TextBox::HandleEvent_IB(sf::Event& event, sf::RenderWindow& window) {
    ...
    void TextBox::setPosition(sf::Vector2f pos) { box.setPosition(pos); text.setPosition(pos.x + 5, pos.y + 8); }
    void TextBox::draw(sf::RenderWindow& window) { window.draw(box); window.draw(text); }
    void TextBox::clear() { input = ""; text.setString(""); }
...
// --- Panels ---
    NodePanel::NodePanel(sf::Font& font, float x, float y)
    ...
    void NodePanel::setPosition(sf::Vector2f pos) {
        bg.setPosition(pos); 
    ...
    PanelResult NodePanel::HandleEvent(sf::Event& event, sf::RenderWindow& window) {
        PanelResult result;
    ...
    void NodePanel::show(VisualNode* node) {
        if (!node) return;
        targetUID = node->UID; isVisible = true; inputField.clear(); 
        infoText.setString("Value: " + std::to_string(node->value) + "\nType: Node");
    }
    void NodePanel::hide() { isVisible = false; targetUID = INT_MIN; }
    void NodePanel::draw(sf::RenderWindow& window) {
        if (!isVisible) return;
    ...
    TimelineSlider::TimelineSlider(sf::Vector2f pos, float width) {
    ...
    void TimelineSlider::HandleEvent(const sf::Event& event, const sf::RenderWindow& window, int& currentFrame, int maxFrames, std::vector<Snapshot>& timeline, std::vector<VisualNode>& nodes, std::vector<Connector>& lines, sf::Font& font, bool& isAutoPlaying, ModernButton& pauseBtn) {
        sf::Vector2f mPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
        if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
    ...
    void TimelineSlider::setPosition(sf::Vector2f pos, float width) {
        trackX = pos.x; trackY = pos.y; trackWidth = width;
        track.setPosition(trackX, trackY); track.setSize({trackWidth, trackHeight});
    }
    void TimelineSlider::update(int currentFrame, int maxFrames) {
        if (maxFrames > 0) {
    ...
    void TimelineSlider::draw(sf::RenderWindow& window, const std::vector<Snapshot>& timeline) {
        window.draw(track);
    ...
    SpeedController::SpeedController(sf::Font& font, float* timeIntervalPtr)
    ...
    void SpeedController::updateTimeInterval() {
        *targetTimeInterval = baseTimeInterval / speedValue;
        label.setString("Speed: " + std::to_string((int)speedValue) + "x");
    }

    void SpeedController::HandleEvent(sf::Event& event, sf::RenderWindow& window) {
        sf::Vector2f mPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
        if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
    ...
    void SpeedController::update(sf::RenderWindow& window) {
        if (isDragging) {
            sf::Vector2f mPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
    ...
    void SpeedController::setPosition(sf::Vector2f pos) {
